...
**To get the current language string** (e.g. for locale-aware logic), use `useLanguage()`:
```tsx
import { useLanguage } from '@/components/shared/LanguageProvider'
const { language } = useLanguage() // 'ky' | 'ru' | 'en'
```
...
`createOutline`, `outlineReady`, `outlineSubtitle`, `coreMessage`, `slideTitle`, `regenerateOutline`, `backToInput`, `generateFromOutline`, `attachFile`, `attachedFiles`, `removeFile`, `rateLimitError`, `rateLimitHint`, `getApiKey`, `apiKeyHint`, `invalidApiKeyError`, `aiModel`, `modelFast`, `modelBalanced`, `modelPowerful`.
...
- `gemini.ts` — `generateAndSavePresentation`, `generateSingleSlideAction`, `generateOutlineAction`, `textAiAction`
  - `generateOutlineAction(prompt, slideCount, tone, audience, customApiKey?, fileContext?, imageFiles?, modelId?)` — returns `{ success, data: OutlineItem[], imageDecisions: ImageDecision[] }`
  - `generateSingleSlideAction(outlineItem, colorTheme, customApiKey?, presentationId?, slideIndex?, modelId?)` — colors driven by `COLOR_PALETTES[colorTheme]`
...
- `/api/parse-document` — document text extraction (`POST`, field name **`file`**). Accepts PDF and DOCX files up to 20MB. Returns `{ text, pages?, chars }`. Uses `pdf-parse` (v2.x `PDFParse` class) for PDF, `mammoth` for DOCX. Text is capped at 20,000 chars. Both packages are in `serverExternalPackages` in `next.config.js` to avoid bundling issues.
...
- File attachment button (paperclip): accepts images (PNG/JPG/WEBP), text files (.txt/.md), **PDF (.pdf)**, and **DOCX (.docx)**
  - Images → uploaded via `POST /api/upload`, stored as URL in `AttachedFile.content`
  - Text files (.txt/.md) → read in browser via `file.text()`, capped at **15,000** chars
  - PDF/DOCX → sent to `POST /api/parse-document`, extracted text capped at **20,000** chars; shows page count for PDF
  - All files appear **immediately** as chips with a spinner while processing; chips show type badge (PDF/DOCX/IMG/TXT), page count or char count, and error state if parsing fails
- Advanced Settings collapse:
  - **AI Model selector** — 4 cards showing available Gemini models (see below)
  - Slide count (3–15)
...
interface AttachedFile {
  id: string
  name: string
  type: 'image' | 'text' | 'document'  // 'document' = PDF or DOCX
  content: string
  pages?: number      // PDF only
  chars?: number      // text/document only
  loading?: boolean   // true while upload/parse is in progress
  parseError?: string // set if parsing failed
}
```

**File attachment & AI image decisions:**

Text files and documents (PDF/DOCX) are passed as `fileContext` (plain text) to the outline prompt — AI uses them to inform slide content. `buildTextContext()` collects all `type === 'text' | 'document'` files that are not loading and have no error.
...
Users choose the Gemini model in **Advanced Settings** of Step 1. The `GEMINI_MODELS` constant in `PresentationForm.tsx` defines the available options:

| Model ID | Label | Status | Use case |
|---|---|---|---|
| `gemini-2.5-flash` | Gemini 2.5 Flash | Stable (**default**) | Fast, cost-efficient, ideal for most presentations |
| `gemini-3-flash-preview` | Gemini 3 Flash | Preview (NEW) | Frontier-class performance at lower cost |
| `gemini-2.5-pro` | Gemini 2.5 Pro | Stable | Deep reasoning, complex topics |
| `gemini-3.1-pro-preview` | Gemini 3.1 Pro | Preview (NEW) | Most powerful, highest quality, highest cost |

**Do NOT use `gemini-2.0-flash` or `gemini-2.0-flash-lite`** — both are deprecated and will be shut down.

The selected model is stored in `selectedModel: GeminiModelId` state (defaults to `'gemini-2.5-flash'`) and passed as `modelId` through the full call chain:
- `generateOutlineAction(..., modelId?)` → `generateOutline(..., modelId?)` → `genAI.getGenerativeModel({ model: modelId || 'gemini-2.5-flash' })`
- `generateSingleSlideAction(..., modelId?)` → `generateSingleSlide(..., modelId?)` → same pattern
...
1. `generateOutlineAction` → `generateOutline` in `src/lib/gemini.ts`
   ...
   - Accepts: `prompt, slideCount, tone, audience, customApiKey?, fileContext?, imageFiles?, modelId?`
...
3. For each slide: `generateSingleSlideAction` → `generateSingleSlide`
   ...
   - Accepts: `outlineItem, colorTheme, customApiKey?, presentationId?, slideIndex?, modelId?`
...
`next.config.js` sets `serverActions.bodySizeLimit: '10mb'` to support image upload payloads, and `serverExternalPackages: ['pdf-parse', 'pdfjs-dist', 'mammoth', '@napi-rs/canvas']` to prevent Next.js from bundling native document-parsing modules.
...
- **Deprecated Gemini models** — never use `gemini-2.0-flash` or `gemini-2.0-flash-lite`; both are deprecated. The default model is `gemini-2.5-flash` (stable).
