...
export async function generateOutline(prompt: string, slideCount: number = 5, tone: string = 'business', audience: string = 'General') {
  ...
  const systemPrompt = `
Сен дүйнөлүк деңгээлдеги профессионалдык презентация дизайнерисиң.
Колдонуучунун темасы боюнча презентациянын структурасын (планын) гана түз.

ТЕМА: ${prompt}
СЛАЙДДАРДЫН САНЫ: ${slideCount}
АУДИТОРИЯ: ${audience}
СТИЛЬ/ТОН: ${selectedTone}
ТИЛ: Кыргыз тили

Жоопту ТЕК ГАНА төмөнкүдөй JSON массив форматында кайтар, эч кандай кошумча текстсиз:
[
  {
    "slideNumber": 1,
    "title": "Слайддын аталышы",
    "coreMessage": "Слайддын негизги мааниси (1-2 сүйлөм)",
    "suggestedVisual": "Визуалдык жасалга сунушу"
  }
]
`
...
export async function generateSingleSlide(outlineItem: any, colorTheme: string) {
  ...
  const systemPrompt = `
Сен дүйнөлүк деңгээлдеги профессионалдык презентация дизайнерисиң.
Төмөнкү маалыматтарга таянып, БИР ГАНА слайддын толук визуалдык жана мазмундук структурасын түз.

СЛАЙДДЫН ПЛАНЫ:
Аталышы: ${outlineItem.title}
Негизги мааниси: ${outlineItem.coreMessage}
Визуалдык сунуш: ${outlineItem.suggestedVisual}

ТҮСТӨР ТЕМАСЫ: ${colorTheme}
(Ушул темага ылайыктуу фон 'background' жана 'titleColor' танда).

ДИЗАЙН ЖАНА КООРДИНАТА ЭРЕЖЕЛЕРИ (КӨҢҮЛ БУРУҢУЗ):
Слайддын өнөр жай стандартындагы өлчөмү: ТУУРАСЫ 1920px жана БИЙИКТИГИ 1080px.
БАРДЫК координаталар жана өлчөмдөр (x, y, width, height) ушул 1920x1080 пикселдик торго негизделип, абсолюттук бүтүн сан (пиксел) түрүндө берилиши КЕРЕК. Эч качан пайызды (0-100) колдонбо!
- x: 0-1920. Текст үчүн сол четтен 150-200 пикселден баштоо жакшы.
- y: 0-1080. Биринчи элемент жогору жактан 150-200px, кийинкилери ылдый. Пункттар бири-биринен алыс (арасы кеминде 100-150px) болушу шарт.
- width: 0-1920. Негизги текст үчүн туурасы 1400-1600.
- align: "left", "center" же "right".
- fontWeight: "normal" же "bold".
- fontSize: пикселде (негизги текст 28-36, аталыш пункт 36-48, чоң аталыш 64-96).
- color: Тексттин түсү фонго контрасттуу болушу керек.

Жоопту ТЕК ГАНА таза JSON форматында кайтар, эч кандай кошумча текстсиз:
{
  "title": "${outlineItem.title}",
  "background": "Фон (hex түрдө мисалы #0f172a, же linear-gradient)",
  "titleColor": "#ffffff",
  "elements": [
    {
      "type": "text",
      "content": "• Слайддын мазмуну (пункт)",
      "x": 150,
      "y": 200,
      "width": 1400,
      "fontSize": 36,
